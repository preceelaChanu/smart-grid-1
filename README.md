...
This project is a real-time framework designed to benchmark the performance of **Homomorphic Encryption (HE)** algorithms within a Smart Grid environment. It simulates a realistic privacy-preserving architecture where Smart Meters (Clients) encrypt their energy consumption data before sending it to an Analytics Center (Server).

The system utilizes the **Microsoft SEAL** library (specifically the BFV scheme) to perform analytics (aggregation) on encrypted data, ensuring that user privacy is maintained at all times while allowing utility companies to calculate total load and billing information.
...
graph TD
    ...
    Client -- Encrypts Data --> Data[(Encrypted Data Files)]
    Data -- Loads Ciphertexts --> Server
    Server -- Computes (Aggregation) --> Result[Encrypted Result]
```

1.  **KeyGen (KGC):** Generates Public, Private, and Relinearization keys based on security configurations.
2.  **Client (Smart Meter):** Simulates a smart meter. Reads real-world data (e.g., London Smart Meter dataset), encrypts it using the Public Key, and serializes the ciphertext to simulated network storage.
3.  **Server (Analytics Center):** Loads encrypted data from multiple clients, performs homomorphic computation (e.g., summation), and verifies the result without ever decrypting individual user inputs.
...
├── keygen/
│   └── keygen.cpp       # Source code for Key Generation Center
├── client/
│   └── client.cpp       # Source code for Smart Meter Simulator
├── server/
│   └── server.cpp       # Source code for Analytics Server
...
```json
{
  "poly_modulus_degree": 8192,      // Security level (4096, 8192, 16384)
  "plain_modulus_bits": 20,         // Bit-size of plaintext space
  "num_clients": 10,                // Number of smart meters to simulate
...
```
  ...
  * **poly\_modulus\_degree:** Higher values (e.g., 16384) increase security but increase computation time and ciphertext size.
...
The framework automatically logs the following metrics to the console for every run:
  ...
  * **Encryption Time:** Time taken for the client to encrypt a single data point.
  ...
  * **Computation Time:** Time taken by the server to aggregate all client data.
  * **Memory Usage:** (Implicitly measured via monitoring tools).

## Data Source

The simulation supports real-world data integration. By default, it uses simulated values, but it is designed to ingest data from the **Smart Meters in London** dataset.
